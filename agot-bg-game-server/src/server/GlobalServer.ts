...
import { seatBots } from "./bot/seatBots";
import { getBotLineup } from "../utils/botLineups";
...
export default class GlobalServer {
      ...
      */
      ...
      try {
        ...
        if (message.type == "create-private-chat-room") {
          ...
          this.botManager.tick(entireGame);
        }
      } catch (e) {
  ...
  async createGame(
    ...
    if (this.debug && process.env.BOT_LINEUP) {
      const botLineup = getBotLineup(process.env.BOT_LINEUP);
      seatBots(entireGame, this.botManager, botLineup);
    }
    return entireGame;
  }
